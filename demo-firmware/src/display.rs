use crate::{
    DisplayResources,
    network::{ETHERNET_1_CONFIG, ETHERNET_2_CONFIG},
};
...
pub(crate) async fn task(r: DisplayResources) -> ! {
    ...
    loop {
        ...
        let eth_1_config = ETHERNET_1_CONFIG.lock().await.clone();
        text.write_str("IP 1: ").unwrap();
        match eth_1_config {
            Some(config) => text.write_fmt(format_args!("{}\n", config.address)),
            None => text.write_str("none\n"),
        }
        .unwrap();

        let eth_2_config = ETHERNET_2_CONFIG.lock().await.clone();
        text.write_str("IP 2: ").unwrap();
        match eth_2_config {
            Some(config) => text.write_fmt(format_args!("{}\n", config.address)),
            None => text.write_str("none\n"),
        }
